...
const audio = require(path.join(__dirname + '/audio'));
...
module.exports = {
  init: function () {
    ...
    app.route('/audio').get(function (req, res) {
      res.sendFile(path.join(__dirname + '/../views/audio/index.html'));
    });

    app.route('/audio/profile/set').post(function (req, res) {
      let query = req.body.profile;
      res.send(audio.profile(query));
    });

    app.route('/audio/profile/get').get(function (req, res) {
      res.send(audio.settings());
    });
